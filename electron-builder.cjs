...
const archiveDirectory = path.dirname(markedArchive);
const archiveBaseName = path.basename(markedArchive);
const extractorDirectory = path.join(__dirname, '.runtime-extractor');
if (!fs.existsSync(path.join(extractorDirectory, '7z.exe')) || !fs.existsSync(path.join(extractorDirectory, '7z.dll'))) {
